...
// Endpoint to update JAM_PENGAMBILAN in Google Sheets
...
// Endpoint to fetch Announcement sheet data from Google Sheets or fallback
app.get("/api/announcements", async (req, res) => {
  ...
  try {
    ...
    const parsedAnnouncements = (data.table.rows || []).map((r: any) => {
      const getColVal = (possibleNames: string[], defaultIdx: number) => {
        if (r && r.c) {
